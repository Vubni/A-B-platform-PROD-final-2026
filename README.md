...
Переменные задаются через окружение (`.env` или env). Значения по умолчанию указаны в `config.py` и/или заданы в `docker-compose.yml` для запуска через Docker.

### Быстрые ссылки

- [База данных](#база-данных)
- [Аутентификация и безопасность](#аутентификация-и-безопасность)
- [Эксперименты и события](#эксперименты-и-события)
- [Логирование](#логирование)
- [Переменные окружения Docker](#переменные-окружения-docker-дocker-composeyml)
- [API](#api)
- [Демо-сценарий](#демо-сценарий-happy-path)
- [Роли и аппрувер-группы](#роли-и-аппрувер-группы)
- [Жизненный цикл эксперимента](#жизненный-цикл-эксперимента)
- [Ключевые архитектурные решения](#ключевые-архитектурные-решения-b7-4)
- [Ограничения и упрощения](#ограничения-и-упрощения-b7-9)
- [Карта репозитория](#карта-репозитория-b7-8)
- [Матрица соответствия и C4](#матрица-соответствия-и-c4-b7-3-b7-5b7-7)
- [Инженерная дисциплина](#инженерная-дисциплина-линтинг-и-форматирование)
- [Runbook и наблюдаемость](#runbook-и-наблюдаемость)
- [Запуск](#запуск)
- [Самостоятельность решения и использование ИИ](#самостоятельность-решения)

### База данных

| Переменная | Env | По умолчанию | Описание |
|------------|-----|--------------|----------|
| `DATE_BASE_CONNECT["host"]` | `DB_HOST` | `postgres` | Хост PostgreSQL |
| `DATE_BASE_CONNECT["user"]` | `DB_USER` | `user` | Пользователь БД |
| `DATE_BASE_CONNECT["password"]` | `DB_PASSWORD` | `prod_secret` | Пароль БД |
| `DATE_BASE_CONNECT["database"]` | `DB_NAME` | `prod` | Имя базы данных |

Для корректной работы триггеров схемы (`docker/postgres/01-init.sql` и `schema/*.sql`). Подробнее — Runbook, раздел «База данных и триггеры».

### Аутентификация и безопасность

| Переменная | Env | По умолчанию | Описание |
|------------|-----|--------------|----------|
| `SECRET` | `RANDOM_SECRET` | `AJd27GqoS#gvxp@V` | Секрет для подписи JWT/сессий |
| `AUTH_TOKEN_EXPIRATION` | — | `86400` (24 ч) | Время жизни токена в секундах (константа в коде) |
| — | `SEED_DEMO_USERS` | `1` | Если задана (например `1`), при старте создаются тестовые пользователи, группа аппруверов, флаг `test_feature_flag`, типы событий `demo_*` и метрики. |
...
| `LEARNINGS_REQUIRED_ON_COMPLETE` | `LEARNINGS_REQUIRED_ON_COMPLETE` | `true` | Если `true`, завершение эксперимента (`POST .../complete`) разрешено только при наличии заполненного learning (`is_completed=true`) |
...
### Переменные окружения Docker (`docker-compose.yml`)

Эти переменные задаются в сервисе `backend` при запуске `docker compose up -d`:

| Env | Значение по умолчанию | Описание |
|-----|------------------------|----------|
| `DB_HOST` | `postgres` | Хост PostgreSQL для backend-сервиса |
| `DB_PASSWORD` | `prod_secret` | Пароль БД для backend-сервиса |
| `KAFKA_BOOTSTRAP_SERVERS` | `kafka:9092` | Адрес Kafka для приёма событий |
| `EVENTS_USE_KAFKA` | `false` | Если `true`, `POST /api/v1/events` пишет события в Kafka, а не сразу в БД |
| `AUTOPILOT_EVAL_INTERVAL_SECONDS` | `60` | Интервал пересчёта автопилота по экспериментам |
| `ADMIN_EMAIL` | `admin@example.com` | Email администратора, создаваемого при старте |
| `ADMIN_PASSWORD` | `admin123` | Пароль администратора, создаваемого при старте |
| `ADMIN_FIRST_NAME` | `Admin` | Имя администратора, создаваемого при старте |
| `SEED_DEMO_USERS` | `1` | Включает создание тестовых пользователей и demo‑данных |
| `LEARNINGS_REQUIRED_ON_COMPLETE` | `true` | Требует заполненный learning при завершении эксперимента |
