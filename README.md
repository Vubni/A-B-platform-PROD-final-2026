...
**Предусловия и переменные окружения** (для локального запуска и проверок): см. `backend/README.md` (раздел «Переменные конфигурации», Runbook).

**Структура репозитория** (карта файлов и модулей): см. `backend/README.md` (раздел «Карта репозитория»).
...
Важно: **ответственность за весь код в репозитории несёте вы** — вы должны понимать, что именно добавляете, уметь объяснить решение и проверять результат (корректность, безопасность, крайние случаи).
