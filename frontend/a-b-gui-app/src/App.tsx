...
const createId = (prefix = 'gui') => {
  ...
  fallbackIdCounter += 1
  return `${prefix}-${Date.now().toString(36)}-${fallbackIdCounter.toString(36)}-${Math.random().toString(36).slice(2, 10)}`
}

const randomInt = (minInclusive: number, maxInclusive: number) => {
  const range = maxInclusive - minInclusive + 1
  const cryptoApi = typeof globalThis !== 'undefined' ? globalThis.crypto : undefined
  if (range > 0 && typeof cryptoApi?.getRandomValues === 'function') {
...
const createDemoSubjectId = () => `shopper-${randomInt(1000, 9999)}`
...
type Drafts = {
  ...
  attachment: { experiment_id: string; attachment_id: string }
...
const swaggerEndpoints: EndpointSpec[] = [
  ...
  { method: 'GET', path: '/api/v1/experiments/{id}/attachments', tag: 'Experiment attachments', summary: 'Список файлов эксперимента' },
  { method: 'POST', path: '/api/v1/experiments/{id}/attachments', tag: 'Experiment attachments', summary: 'Загрузить файл к эксперименту', body: 'FormData: file=<выберите файл в разделе экспериментов>' },
  { method: 'GET', path: '/api/v1/experiments/{id}/attachments/{attachment_id}', tag: 'Experiment attachments', summary: 'Скачать файл эксперимента' },
  { method: 'DELETE', path: '/api/v1/experiments/{id}/attachments/{attachment_id}', tag: 'Experiment attachments', summary: 'Удалить файл эксперимента' },
  ...
  { method: 'GET', path: '/api/v1/experiments/{id}/report/html?start=2026-02-01&end=2026-12-31', tag: 'Reports', summary: 'HTML-отчёт по эксперименту через Jinja2-шаблон' },
...
const initialDrafts: Drafts = {
  ...
  attachment: { experiment_id: '', attachment_id: '' },
...
function App() {
  ...
  const api = async (method: HttpMethod, path: string, body?: unknown, options?: { silent?: boolean; authToken?: string }) => {
    ...
    try {
      ...
      if (!options?.silent) {
        pushLog({ method, path: normalizedPath, status: response.status, ok: response.ok, duration: performance.now() - started, request: isFormBody ? describeFormData(body) : body, response: data })
      }
  ...
  const download = async (path: string) => {
    ...
    try {
      ...
      const filename = filenameFromDisposition(response.headers.get('content-disposition')) || `attachment-${Date.now()}`
      if (response.ok) {
      ...
      pushLog({
        ...
        response: { downloaded: response.ok, filename, size: blob.size, type: blob.type || 'application/octet-stream' },
      })
      return { ok: response.ok, status: response.status, data: { filename, size: blob.size, type: blob.type } }
    } catch (error) {
      const data = error instanceof Error ? { error: error.message } : { error: String(error) }
      pushLog({ method: 'GET', path: normalizedPath, status: 'ERR', ok: false, duration: performance.now() - started, response: data })
      return { ok: false, status: 'ERR' as const, data }
    }
  }

  const pushLog = (entry: Omit<RequestLog, 'id' | 'at'>) => {
    setLogs((prev) => [{ ...entry, id: createId('log'), at: new Date().toLocaleTimeString('ru-RU') }, ...prev].slice(0, 30))
  }
  ...
  const runScenario = async () => {
    ...
    updateDraft('attachment', { experiment_id: experimentId })
  ...
  const module = useMemo(() => {
    const common = { api, download, catalogs, drafts, updateDraft, refreshAll, register }
    switch (active) {
      case 'overview':
        return (
          <Overview
...
type ModuleProps = {
  ...
  download: (path: string) => Promise<ApiResult>
  catalogs: Catalogs
  drafts: Drafts
...
function ExperimentsModule({ api, download, catalogs, drafts, updateDraft, refreshAll, activeExperiment }: ModuleProps & { activeExperiment?: ExperimentItem }) {
  const d = drafts
  const [attachmentFile, setAttachmentFile] = useState<File | null>(null)
  const [attachments, setAttachments] = useState<Array<Record<string, unknown>>>([])
  const [attachmentsLoadedFor, setAttachmentsLoadedFor] = useState('')
  const [attachmentsLoading, setAttachmentsLoading] = useState(false)

  const loadAttachments = async (experimentId = d.attachment.experiment_id) => {
    if (!experimentId) {
      setAttachments([])
      setAttachmentsLoadedFor('')
      return
    }
    setAttachmentsLoading(true)
    try {
      const res = await api('GET', `/api/v1/experiments/${experimentId}/attachments`)
      if (res.ok) {
        setAttachments(extractAttachmentRows(res.data))
        setAttachmentsLoadedFor(experimentId)
      }
    } finally {
  ...
  const chooseAttachmentExperiment = (experimentId: string) => {
    updateDraft('attachment', { experiment_id: experimentId, attachment_id: '' })
    void loadAttachments(experimentId)
  }

  const deleteAttachment = async (row: Record<string, unknown>) => {
    const experimentId = d.attachment.experiment_id
    const id = attachmentId(row)
    if (!experimentId || !id) return
    await api('DELETE', `/api/v1/experiments/${experimentId}/attachments/${id}`)
    await loadAttachments(experimentId)
  }

  const downloadAttachment = async (row: Record<string, unknown>) => {
    const experimentId = d.attachment.experiment_id
    const id = attachmentId(row)
    if (!experimentId || !id) return
    await download(`/api/v1/experiments/${experimentId}/attachments/${id}`)
  }
  ...
  return (
    <>
      ...
      <div className="two-col">
        <FormCard
          title="Файлы эксперимента"
          submitLabel={attachmentFile ? 'Загрузить файл' : 'Обновить список'}
          ...
          onSubmit={async () => {
            ...
            const res = await api('POST', `/api/v1/experiments/${d.attachment.experiment_id}/attachments`, body)
            if (res.ok) {
        ...
        >
          <SelectInput label="Эксперимент" value={d.attachment.experiment_id} options={catalogs.experiments.map(expOption)} onChange={chooseAttachmentExperiment} />
          <label className="field">
            Файл
            <input type="file" onChange={(event) => setAttachmentFile(event.target.files?.[0] || null)} />
          </label>
          <div className="button-row">
            <button type="button" className="secondary" onClick={() => loadAttachments()} disabled={attachmentsLoading}>
              <RefreshCw size={16} />
              {attachmentsLoading ? 'Загрузка...' : 'Обновить список'}
            </button>
          </div>
          <div className="field-notes">
            <span>Выберите эксперимент: список файлов появится ниже. После загрузки или удаления таблица обновится автоматически.</span>
          </div>
        </FormCard>
        <section className="panel attachment-help">
          <div className="panel-title">
            <h2>Работа без ID</h2>
            <span>{attachmentsLoadedFor ? short(attachmentsLoadedFor) : 'эксперимент не выбран'}</span>
          </div>
          <p>Файлы ниже можно скачивать и удалять прямо из строк таблицы. ID остаются видимыми только для сверки с backend и не требуют ручного ввода.</p>
          <div className="attachment-kpis">
            <div><span className="caption">Файлов</span><strong>{attachments.length}</strong></div>
            <div><span className="caption">Статус</span><strong>{attachmentsLoading ? 'загрузка' : attachmentsLoadedFor ? 'загружено' : 'ожидает выбора'}</strong></div>
          </div>
        </section>
      </div>
      <DataTable
        ...
        rows={attachments}
        columns={attachmentColumns(attachments)}
        empty={d.attachment.experiment_id ? 'Файлы не загружены или у эксперимента пока нет attachments.' : 'Выберите эксперимент, чтобы увидеть прикреплённые файлы.'}
        formatCell={(column, value) => attachmentCell(column, value)}
        actions={(row) => (
          <div className="row-actions">
            <button className="mini" onClick={() => downloadAttachment(row)} disabled={!attachmentId(row)}>
              <Download size={14} />
              скачать
            </button>
            <button className="mini danger" onClick={() => deleteAttachment(row)} disabled={!attachmentId(row)}>
              <Archive size={14} />
              удалить
            </button>
          </div>
      ...
      <DataTable
        ...
        actions={(row) => (
          <div className="row-actions">
              ...
              updateDraft('attachment', { experiment_id: id })
    ...
    <>
      ...
      <div className="two-col">
        ...
        <FormCard
          ...
          onSubmit={async () => {
            await api('POST', '/api/v1/events', { events: [{ event_id: createId('gui'), decision_id: d.event.decision_id, event_type_key: d.event.event_type_key, subject_id: d.event.subject_id, timestamp: new Date().toISOString(), payload: parseJson(d.event.payload) }] })
          }}
        >
...
function AbDemoModule({ api, catalogs, updateDraft, onOpenReports, onPrepareDemo }: ModuleProps & { onOpenReports: () => void; onPrepareDemo: () => Promise<{ ok: boolean; flagKey: string; experimentId: string; message: string }> }) {
  ...
  const [subjectId, setSubjectId] = useState(createDemoSubjectId)
    ...
    if (res.ok && nextAssignment) {
      ...
      if (nextDecisionId) {
        await api('POST', '/api/v1/events', {
          events: [{
            event_id: createId('gui-demo-exposure'),
  ...
  const sendConversion = async () => {
    ...
    await api('POST', '/api/v1/events', {
      events: [{
        event_id: createId('gui-demo-click'),
  ...
  const generateDemoTraffic = async () => {
    ...
    try {
      ...
      const batchId = createId('batch').slice(-8)
      for (let index = 0; index < 18; index += 1) {
        ...
        const events = [{
          event_id: createId('gui-demo-traffic-exposure'),
          decision_id: nextDecisionId,
          event_type_key: 'demo_exposure',
          subject_id: nextSubjectId,
          timestamp: new Date().toISOString(),
        ...
        if (shouldClick) {
          events.push({
            event_id: createId('gui-demo-traffic-click'),
            decision_id: nextDecisionId,
            event_type_key: 'demo_click',
            subject_id: nextSubjectId,
            timestamp: new Date().toISOString(),
        ...
        <div className="demo-control">
          ...
          <div className="button-row">
            ...
            <button className="secondary" onClick={() => {
              setSubjectId(createDemoSubjectId())
              setAssignment(null)
...
function ReportsModule({ api, catalogs, drafts, updateDraft }: ModuleProps) {
  ...
  const reportQuery = `start=${encodeURIComponent(d.start)}&end=${encodeURIComponent(d.end)}`
  return (
    <>
      ...
      <FormCard
        ...
        onSubmit={async () => {
          const res = await api('GET', `/api/v1/experiments/${d.experiment_id}/report?${reportQuery}&include_dynamics=${d.include_dynamics}`)
      ...
      >
        ...
        <div className="button-row">
          <button
            ...
            className="secondary"
            onClick={async () => {
              const res = await api('GET', `/api/v1/experiments/${d.experiment_id}/report/html?${reportQuery}`)
...
function RawModule({ api, drafts, updateDraft }: ModuleProps) {
  ...
  const tags = Array.from(new Set(swaggerEndpoints.map((endpoint) => endpoint.tag))).sort()
  const endpointRows = swaggerEndpoints
    .filter((endpoint) => methodFilter === 'ALL' || endpoint.method === methodFilter)
    .filter((endpoint) => tagFilter === 'ALL' || endpoint.tag === tagFilter)
    .filter((endpoint) => {
      const haystack = `${endpoint.method} ${endpoint.path} ${endpoint.tag} ${endpoint.summary}`.toLowerCase()
  ...
  return (
    <>
      ...
      <section className="panel endpoint-toolbar">
        <TextInput label="Поиск по endpoint-ам" value={query} onChange={setQuery} />
        <SelectInput label="Method" value={methodFilter} options={['ALL', 'GET', 'POST', 'PATCH', 'PUT', 'DELETE']} onChange={setMethodFilter} />
        <SelectInput label="Tag" value={tagFilter} options={['ALL', ...tags]} onChange={setTagFilter} />
      </section>
      <DataTable
        title={`Каталог Swagger endpoints: ${endpointRows.length} из ${swaggerEndpoints.length}`}
        rows={endpointRows}
...
function describeFormData(value: FormData) {
  return Object.fromEntries(Array.from(value.entries()).map(([key, entry]) => [
    key,
    entry instanceof File ? { name: entry.name, size: entry.size, type: entry.type || 'application/octet-stream' } : entry,
  ]))
}
...
function extractAttachmentRows(value: unknown) {
  const direct = arrayFrom(value, 'attachments')
  if (direct.length > 0) return direct.filter(isRecord)
  const files = arrayFrom(value, 'files')
  if (files.length > 0) return files.filter(isRecord)
  const items = arrayFrom(value, 'items')
  if (items.length > 0) return items.filter(isRecord)
  return Array.isArray(value) ? value.filter(isRecord) : []
}

function attachmentId(row: Record<string, unknown>) {
  return String(row.id || row.attachment_id || row.file_id || row.uuid || '')
}

function attachmentColumns(rows: Array<Record<string, unknown>>) {
  const preferred = ['filename', 'file_name', 'name', 'content_type', 'mime_type', 'size', 'created_at', 'uploaded_at', 'id', 'attachment_id']
  const available = new Set(rows.flatMap((row) => Object.keys(row)))
  const columns = preferred.filter((key) => available.has(key))
  if (columns.length > 0) return columns.slice(0, 6)
  return ['id', 'name', 'size', 'created_at']
}

function attachmentCell(column: string, value: unknown) {
  if (column === 'size' && typeof value === 'number') return formatBytes(value)
  if ((column === 'created_at' || column === 'uploaded_at') && typeof value === 'string') return formatReportValue(value)
  return cell(value)
}
...
function hydrateDraftIds(drafts: Drafts, catalogs: Catalogs): Drafts {
  ...
  return {
    ...
    attachment: { ...drafts.attachment, experiment_id: drafts.attachment.experiment_id || firstExperiment },
