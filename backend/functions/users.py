...
async def get_users_list(role: str | None = None) -> list[dict]:
...
async def create_user(
    email: str, first_name: str, password: str, role: str = "viewer"
) -> dict | None:
    async with Database() as db:
...
async def get_user_by_email(email: str) -> dict | None:
    async with Database() as db:
        row = await db.execute(
            "SELECT id, email, first_name, role, verified, password, created_at, updated_at FROM users WHERE email = $1",
            (email,),
        )
        return serialize_json(row)
...
async def create_approver_group(
    experimenter_id: str | None, min_approvals: int, approver_ids: list[str]
) -> dict | None:
    async with Database() as db:
        if experimenter_id is not None:
            user = await db.execute("SELECT id, role FROM users WHERE id = $1", (experimenter_id,))
            if not user:
                return None
        existing = await db.execute(
        ...
        if existing:
            return None
        await db.execute(
            "INSERT INTO approver_groups (experimenter_id, min_approvals) VALUES ($1, $2)",
...
async def update_approver_group(
    group_id: str, min_approvals: int | None = None, approver_ids: list[str] | None = None
) -> dict | None:
    async with Database() as db:
        row = await db.execute(
