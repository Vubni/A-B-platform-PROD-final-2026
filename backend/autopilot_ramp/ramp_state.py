...
async def start_autopilot(experiment_id: str) -> tuple[dict | None, str | None]:
    async with Database() as db:
        plan = await db.execute("SELECT id FROM ramp_plans WHERE experiment_id = $1", (experiment_id,))
        if not plan:
        ...
        existing = await db.execute(
            "SELECT 1 FROM experiment_ramp_state WHERE experiment_id = $1",
            (experiment_id,))
        if existing:
...
async def get_ramp_decision_log(
    experiment_id: str, limit: int = 100
) -> list[dict]:
    async with Database() as db:
